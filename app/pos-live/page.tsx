...
type CartItem = SaleItem & {
  ...
  isFreeGift?: boolean
  isDelivered?: boolean // 是否已出貨（品項級別）
}
...
export default function POSPage() {
  ...
  // 切換單個品項的出貨狀態
  const toggleItemDelivered = (index: number) => {
    setCart((prev) =>
      prev.map((item, i) => {
        if (i === index) {
          return {
    ...
    try {
      ...
      const res = await fetch('/api/sales', {
        ...
        body: JSON.stringify({
          ...
          delivery_note: !allItemsDelivered ? deliveryNote : undefined, // 有未出貨時保存備註
          note: note || undefined,
          ...
          items: checkoutCart.map((item) => ({
            ...
            is_delivered: item.isDelivered !== false, // 品項級別的出貨狀態
          })),
          // 點數計劃（有選擇客戶時才傳送）
  ...
  if (isMobile) {
    return <MobilePOS salesMode="live" />
  }
          ...
          <div className="flex-1 bg-slate-900 flex flex-col border-r border-slate-700">
                      ...
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex-1">
                          <div className="font-medium text-sm text-white">
                            ...
                            {!(cart[item.indices![0]]?.isDelivered ?? true) && (
                          ...
                          {!item.ichiban_kuji_id && (
                            <div className="flex items-center gap-3 mt-1">
                              ...
                              <label className="flex items-center gap-1 cursor-pointer">
                                <input
                                  type="checkbox"
                                  checked={!(cart[item.indices![0]]?.isDelivered ?? true)}
                                  onChange={() => toggleItemDelivered(item.indices![0])}
                                  className="w-3 h-3 rounded border-gray-300 text-orange-500 focus:ring-orange-500 accent-orange-500"
                                />
                                <span className="text-xs text-orange-500">未出貨</span>
                              </label>
                            </div>
                          )}
                        </div>
            ...
            <div className="bg-slate-800 border-t border-slate-700 p-4">
              ...
              <div className="border-t border-slate-600 pt-3 flex justify-between items-center">
                <span className="text-lg text-slate-300">總計</span>
