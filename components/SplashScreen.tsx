...
export default function SplashScreen({ onFinish }: SplashScreenProps) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center overflow-hidden bg-gradient-to-br from-gray-900 via-slate-800 to-gray-900">
      ...
      {/* 主要內容 */}
      <div
        className={`relative z-10 text-center transition-all duration-1000 ${stage >= 2 ? 'scale-110 opacity-0' : 'scale-100 opacity-100'
          }`}
      >
        {/* Logo 容器 */}
        <div
          className={`mb-8 flex justify-center transition-all duration-1000 ${stage >= 0 ? 'scale-100 opacity-100 rotate-0' : 'scale-0 opacity-0 -rotate-12'
            }`}
        >
        ...
        {/* 文字動畫 */}
        <div
          className={`mb-8 transition-all duration-1000 delay-300 ${stage >= 1 ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
            }`}
        >
        ...
        {/* 進度條 */}
        <div
          className={`mx-auto w-64 transition-all duration-500 delay-500 ${stage >= 1 ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
            }`}
        >
          <div className="h-1.5 overflow-hidden rounded-full bg-white/20 backdrop-blur-sm">
            <div
              className={`h-full bg-gradient-to-r from-yellow-400 via-yellow-200 to-yellow-400 transition-all duration-[2800ms] ease-out ${stage >= 1 ? 'w-full' : 'w-0'
                }`}
